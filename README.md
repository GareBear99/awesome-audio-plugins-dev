# Awesome Audio Plugin Development

A curated list of **free audio plugins**, DSP tools, and resources for
building audio plugins (VST3, AU, CLAP, LV2).\
Focused on **plugin developers, DSP engineers, and open‑source music
technology**.

Inspired by the awesome-list movement.

------------------------------------------------------------------------

## ⭐ Featured Plugin Ecosystem

### TizWildin Entertainment HUB

Central hub for experimental audio plugins and DSP tools by TizWildin/GareBear99.

https://github.com/GareBear99/TizWildinEntertainmentHUB

### Featured Plugins

-   **FreeEQ8** -- Free open‑source 8‑band parametric EQ plugin
    (VST3/AU)\
    https://github.com/GareBear99/FreeEQ8

-   **PaintMask** -- Visual paint‑based audio processing plugin.

-   **RiftWave Suite** -- Lightweight modular synthesizer engine.

-   **MAID Suite** -- Modular Audio Innovation & Development toolkit.

-   **FreeSampler** -- Lightweight High-Performance Audio Sampler Plugin (JUCE)
  https://github.com/GareBear99/FreeSampler_v0.3

------------------------------------------------------------------------

# Equalizers

-   **TDR Nova** -- Dynamic parametric EQ\
    https://www.tokyodawn.net/tdr-nova/

-   **TDR SlickEQ** -- Musical mixing EQ\
    https://www.tokyodawn.net/tdr-slickeq/

-   **MEqualizer** -- Parametric EQ\
    https://www.meldaproduction.com/MEqualizer

-   **Luftikus** -- Analog‑style mastering EQ\
    https://github.com/kxstudio/Luftikus

-   **Blue Cat Triple EQ**\
    https://www.bluecataudio.com/Products/Product_TripleEQ/

------------------------------------------------------------------------

# Synthesizers

-   **Vital** -- Advanced wavetable synthesizer\
    https://vital.audio

-   **Surge XT** -- Open‑source hybrid synthesizer\
    https://surge-synthesizer.github.io

-   **Dexed** -- Yamaha DX7 emulation\
    https://github.com/asb2m10/dexed

-   **Helm** -- Cross‑platform synth\
    https://tytel.org/helm/

-   **Odin2** -- Hybrid synthesizer\
    https://thewavewarden.com/odin2/

------------------------------------------------------------------------

# Compression

-   **TDR Kotelnikov** -- Mastering compressor\
    https://www.tokyodawn.net/tdr-kotelnikov/

-   **RoughRider3** -- Character compressor\
    https://www.audiodamage.com/products/roughrider3

-   **DC1A** -- Simple compressor\
    https://klanghelm.com/contents/products/DC1A.php

-   **MJUC jr.** -- Vari‑mu compressor\
    https://klanghelm.com/contents/products/MJUCjr.php

------------------------------------------------------------------------

# Saturation / Distortion

-   **Softube Saturation Knob**\
    https://www.softube.com/saturationknob

-   **CamelCrusher**\
    https://www.audiopluginsforfree.com/camelcrusher/

-   **Klanghelm IVGI**\
    https://klanghelm.com/contents/products/IVGI.php

-   **ChowTapeModel** -- Tape saturation\
    https://github.com/jatinchowdhury18/AnalogTapeModel

------------------------------------------------------------------------

# Reverb

-   **OrilRiver**\
    https://www.kvraudio.com/product/orilriver-by-denis-tihanov

-   **Dragonfly Reverb**\
    https://github.com/michaelwillis/dragonfly-reverb

-   **TAL Reverb 4**\
    https://tal-software.com/products/tal-reverb-4

------------------------------------------------------------------------

# Delay

-   **Valhalla Supermassive**\
    https://valhalladsp.com/shop/reverb/valhalla-supermassive/

-   **TAL Dub III**\
    https://tal-software.com/products/tal-dub-3

-   **Spaceship Delay**\
    https://www.kvraudio.com/product/spaceship-delay-by-musical-entropy

------------------------------------------------------------------------

# Creative Effects

-   **Fracture**\
    https://glitchmachines.com/products/fracture/

-   **Hysteresis**\
    https://glitchmachines.com/products/hysteresis/

-   **PaulXStretch**\
    https://github.com/essej/paulxstretch

------------------------------------------------------------------------

# Utility / Metering

-   **SPAN**\
    https://www.voxengo.com/product/span/

-   **Youlean Loudness Meter**\
    https://youlean.co/youlean-loudness-meter/

-   **MLoudnessAnalyzer**\
    https://www.meldaproduction.com/MLoudnessAnalyzer

-   **HOFA 4U Meter**\
    https://hofa-plugins.de/en/plugins/4u/

------------------------------------------------------------------------

# Modulation

-   **TAL Chorus LX**\
    https://tal-software.com/products/tal-chorus-lx

-   **Blue Cat Chorus**\
    https://www.bluecataudio.com/Products/Product_Chorus/

------------------------------------------------------------------------

# Filters

-   **TAL Filter 2**\
    https://tal-software.com/products/tal-filter-2

-   **HY Filter4 Free**\
    https://hy-plugins.com/product/hy-filter4-free/

------------------------------------------------------------------------

# Open‑Source DSP Plugins

-   **ChowMatrix**\
    https://github.com/jatinchowdhury18/ChowMatrix

-   **ChowPhaser**\
    https://github.com/jatinchowdhury18/ChowPhaser

-   **Wolf Shaper**\
    https://github.com/wolf-plugins/wolf-shaper

-   **Airwindows Plugins**\
    https://github.com/airwindows/airwindows

------------------------------------------------------------------------

# Contributing

Contributions are welcome.

You can submit:

-   free plugins
-   open‑source DSP tools
-   plugin development frameworks
-   audio engineering resources

Open a pull request to add new tools.

------------------------------------------------------------------------

# License

MIT
